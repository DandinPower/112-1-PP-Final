...
template <typename index_t, typename scalar_t, typename index_t_ptr, typename scalar_t_ptr>
void thread_matmul(const int start_row, const int end_row, const index_t_ptr Ap, const index_t_ptr Aj, const scalar_t_ptr Ax, const index_t_ptr Bp, const index_t_ptr Bj, const scalar_t_ptr Bx, std::vector<std::vector<index_t>> &next, std::vector<std::vector<scalar_t>> &sums, std::vector<index_t> &head, std::vector<index_t> &length) {
    ...
    typename scalar_t_ptr::value_type Cx[])
{
  ...
  for (int t = 0; t < THREADS; t++)
  {
    ...
    threads[t] = std::thread(thread_matmul<index_t, scalar_t, index_t_ptr, scalar_t_ptr>, start_row[t], end_row[t], Ap, Aj, Ax, Bp, Bj, Bx, std::ref(next), std::ref(sums), std::ref(head), std::ref(length));
  }
  for (int t = 0; t < THREADS; t++)
  {
    threads[t].join();
...
torch::Tensor sparse_matmul_kernel_std_thread(
    torch::Tensor output,
...
torch::Tensor sparse_sparse_matmul_cpu_std_thread(const torch::Tensor &mat1_, const torch::Tensor &mat2_)
{
  ...
  auto answer = AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(mat1_.scalar_type(), "sparse_matmul", [&]() -> at::Tensor
                                                       { return sparse_matmul_kernel_std_thread<scalar_t>(output, mat1_.coalesce(), mat2_.coalesce()); });
