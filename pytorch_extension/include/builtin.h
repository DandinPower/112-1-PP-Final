...
void _csr_matmult(const int64_t n_row, const int64_t n_col,
                  ...
                  const scalar_t_ptr Ax, const index_t_ptr Bp,
                  const index_t_ptr Bj, const scalar_t_ptr Bx,
    ...
    for (int i = 0; i < n_row; i++) {
        ...
        auto col_indices_accessor =
            at::native::StridedRandomAccessor<int64_t>(Cj + nnz - length, 1);
        auto val_accessor =
            at::native::StridedRandomAccessor<scalar_t>(Cx + nnz - length, 1);
...
torch::Tensor sparse_matmul_kernel(torch::Tensor output,
    ...
    _csr_matmult(M, N, mat1_crow_indices_ptr, mat1_col_indices_ptr,
    ...
    copy_value<int64_t>(nnz * 2, indices.data_ptr<int64_t>(),
                        output._indices().data_ptr<int64_t>());
    copy_value<scalar_t>(nnz, values.data_ptr<scalar_t>(),
                         output._values().data_ptr<scalar_t>());
...
torch::Tensor sparse_sparse_matmul_cpu(const torch::Tensor &mat1_,
    ...
    output.sparse_resize_and_clear_({mat1_.size(0), mat2_.size(1)},
                                    mat1_.sparse_dim(), 0);

    auto answer = AT_DISPATCH_FLOATING_AND_COMPLEX_TYPES(
        mat1_.scalar_type(), "sparse_matmul", [&]() -> at::Tensor {
            return sparse_matmul_kernel<scalar_t>(output, mat1_.coalesce(),
                                                  mat2_.coalesce());
        });
